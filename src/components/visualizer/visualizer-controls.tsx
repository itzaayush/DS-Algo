...
import type { VisualizerController } from "@/hooks/use-visualizer";
...
export function VisualizerControls({ c }: { c: VisualizerController }) {
  const counters = c.frame?.counters ?? {};
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-3">
        <span className="font-mono text-xs text-muted tabular-nums">
          ...
          <span className="text-faint">/{String(c.total - 1).padStart(2, "0")}</span>
        </span>
        <input
          type="range"
          min={0}
          ...
          aria-label="Scrub algorithm step"
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <Tooltip content={c.playing ? "Pause (Space)" : "Play (Space)"}>
